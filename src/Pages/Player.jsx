...
function Player() {
  ...
  useEffect(() => {
    ...
    const fetchComment = async function () {
      try {
        const response = await fetch(`https://antonpklive.online/v1/api/user/user/video/comment/${videoId}?sort=latest`,
          {
            method: "GET",
            credentials: "include"
          }
    ...
    const fecthLikes = async function () {
      try {
        const response = await fetch(`https://antonpklive.online/v1/api/user/user/video/${videoId}/get-alllikes`,
          {
            method: "POST",
            credentials: "include",
            headers: {
  ...
  const loadMoreComment = useCallback(async function () {
    if (!hasMore || loading) {
      ...
      try {
        console.log("called");
        setLoading(true)
        const response = await fetch(`https://antonpklive.online/v1/api/user/user/video/comment/${videoId}?cursor=${cursor}`,
          {
            method: "GET",
            credentials: "include"
          }
  ...
  }, [hasMore, loading, cursor, videoId])
